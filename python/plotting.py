...
#       von Mises fitting
        ...
        fig.savefig(
                '{}{}_angularvelocity_{}-{}.png'.format(self.figpath,
                self.filename, self.timebins, n),
                bbox_inches = 'tight')
        fig1.savefig(
                '{}{}_angularvelocity_cumsum_{}-{}.png'.format(
                    self.figpath, self.filename, self.timebins, n
                    ),
                bbox_inches = 'tight')
        ...
        ax2 = fig2.subplots()
        ax2.plot(bincenters,
                binheight - np.flip(binheight),
                label = 'Left-Right')
        ax2.legend()

        fig2.savefig('{}{}_avsymmetry_{}-{}.png'.format(self.figpath,
                    self.filename,
                    self.timebins,
                    n),
                bbox_inches = 'tight')
    ...
    def plot_dfc_hist(self, dfc, n):
        ...
        binheight, histbins, _ = ax[0].hist(
                                    dfc, self.plotres, 
                                    density = self.density,
                                    label = 'Data')
        binheight, histbins, _ = ax[1].hist(
                                    dfc, self.plotres, 
                                    density = self.density,
                                    label = 'Data')
        binheight, histbins, _ = ax[2].hist(
                                    dfc, self.plotres, 
                                    density = self.density,
                                    label = 'Data')
        ...
        ax[0].plot(plotbins, 
                   composite_lorentz_polyo1(plotbins, *popt), 
        ...
        plt.savefig('{}{}_dfc_{}-{}.png'.format(self.figpath, self.filename,
                                                self.timebins, n)
        plt.close()
    ...
    def plot_displacement(self, displacement,n):
        ...
        plt.savefig('{}{}_displacement_{}-{}.png'.format(
                                                    self.figpath,
                                                    self.filename,
                                                    self.timebins, n),
                    bbox_inches = 'tight')
    ...
    def plot_orientation_hist(self, orientation, n):
        ...
        ax.hist(orientation,
                bins = np.linspace(-math.pi, math.pi, 46),
                label = 'Single ant speed',
                density = self.density)
        ax1.hist(orientation,
                 bins = np.linspace(-math.pi, math.pi, 91),
                 label = 'Single ant speed',
                 density = self.density,
                 cumulative = True)
        ...
        ax.set_ylabel('frequency')
        fig.savefig(
                '{}{}_orientation_{}-{}.png'.format(self.figpath,
                self.filename, self.timebins, n), bbox_inches = 'tight')
        fig1.savefig(
                '{}{}_orientation_cum_{}-{}.png'.format(self.figpath,
                self.filename, self.timebins, n), bbox_inches = 'tight')
        plt.close(fig)
        plt.close(fig1)
    ...
    def plot_position_hist(self, position, n):
        ...
        h = ax.hist2d(position[0], position[1],self.plotres, 
                      label = 'Single ant position',
                      norm=LogNorm())
        #plt.colorbar(h[3])
        #plt.scatter(position[0], position[1], alpha = 0.005 * self.timebins)
        plt.xlabel('x (cm)')
        plt.ylabel('y (cm)')
        plt.savefig('{}{}_2dhist_{}-{}.png'.format(
                                                self.figpath,
                                                self.filename,
                                                self.timebins,
                                                n),
    ...
    def plot_speed_hist(self, speed, n):
        ...
        ax = fig.add_subplot()
        ax.hist(speed,
                bins = np.linspace(min(speed), max(speed), self.plotres),
                label='Single ant speed',
                density = self.density)
        temp = -speed
        ...
        #ax.set_xlim([0, 5])
        ax.set_ylim([0, 1.2])
        fig.savefig('{}{}_speed_hist_{}-{}.png'.format(
                                                    self.figpath,
                                                    self.filename,
                                                    self.timebins,
                                                    n),
    ...
    def plot_partialtraj(self, position, n):
        partialtraj = self.partialtraj
        cutrange = np.arange(len(partialtraj))
        partialtraj = cutrange[partialtraj]
        ...
        ax.set_xlabel('x (cm)')
        ax.set_ylabel('y (cm)')
        fig.savefig('{}{}_trajectory_{}-{}.png'.format(
                                                    self.figpath,
                                                    self.filename,
                                                    self.timebins,
                                                    n),
    ...
    def plot_trajectory(self, position, n):
        ...
        plt.savefig('{}{}_trajectory_{}-{}.png'.format(
                                                    self.figpath,
                                                    self.filename,
                                                    self.timebins,
                                                    n),
    ...
    def plot_velocity(self, velocity, n):
        ...
        ax.set_xlabel('x (cm)')
        ax.set_ylabel('y (cm)')
        plt.savefig('{}{}_velocity_{}-{}.png'.format(
                                                self.figpath,
                                                self.filename,
                                                self.timebins,
                                                n),
    ...
    def plot_orient_vs_dist_from_center(self, data1, data2, n):
        ...
        plt.savefig('{}{}_radialdistance{}{}.png'.format(
                                                    self.figpath,
                                                    self.filename,
                                                    self.timebins,
                                                    n),
    ...
    def plot_dfc_vs_speed(self, dfc, speed, n):
        ...
        plt.savefig('{}{}_dfcvsspeed{}{}.png'.format(
                                                self.figpath,
                                                self.filename,
                                                self.timebins,
                                                n),
    ...
    class Trajectory:
        ...
        def make_indices(self, data, criteria):
            ...
            truthfunction = (((data[:, 0] > minthresh) 
                              & (data[:, 0] < maxthresh)) 
                             & ((data[:, 1] > minthresh) 
                                & (data[:, 1] < maxthresh)))
    ...
    for setname in ap.datafile: 
        ...
        if ap.runtype == 's':
            for t in range(ap.timebins):
                ...
                indices = np.empty((len(trajectories[-1].position)),
                                   dtype = bool)
                indices.fill(False)
                if ap.boundary:
                ...
                ap.plot_position_hist(trajectories[-1].position[indices],
                                      t  + 1)
                ap.plot_speed(trajectories[-1].speed[indices[1:]], t + 1)
                ap.plot_speed_hist(trajectories[-1].speed[indices[1:]],
                                   t + 1)
                ap.plot_orientation_hist(trajectories[-1].orientation[
                                                            indices[1:]],
                                         t + 1)
        ...
        for t in range(ap.timebins):
            ...
            for traj in trajectories:
                ...
                pool.orientation = np.append(pool.orientation, 
                                             traj.orientation[indices],
                                             axis = 0)
                pool.angularvelocity = np.append(
                                            pool.angularvelocity, 
                                            traj.angularvelocity[indices],
                                            axis = 0)
                pool.acceleration = np.append(pool.acceleration, 
                                              traj.acceleration[indices],
                                              axis = 0)
